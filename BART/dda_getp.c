...
static const char* usage_str = "<input s [ny nz nt]> <output p>";
static const char* help_str = 	"Get the differential distribution for a sampling pattern\n";
...
int main_dda_getp(int argc, char* argv[])
{
    ...
    long dims[DIMS];
    complex float* pat = load_cfl(argv[1], DIMS, dims);
    ...
    complex float *diff_dist = create_cfl(argv[2], DIMS, dimsp);
