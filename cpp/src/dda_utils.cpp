...
void printWsp(const SparseW *wsp){
    ...
    for( int j = 0 ; j < wsp->len_w[t1][t2] ; j++ ){
        debug_printf(DP_INFO, "w(");
        for( int i = 0 ; i < D ; i++ ){
            debug_printf(DP_INFO, "%d, ", wsp->kw[t1][t2][j*D + i]);
        }
...
void sparsifyW(const long D, SparseW *wsp, MDArray<double> *wmd, const double T){
    ...
    memcpy(wsp->dims, wmd->dims, DIMS*sizeof(long));

    // set wsp->w00tt
    for( long t = 0 ; t < nt ; t++ ){
        wsp->w00tt[t] = 0;
        wsp->w00tt[t] = wsp->w00tt[t] + wmd->data[t * strs[D] + t*strs[D+1]];
    }
    ...
    for( long t1 = 0 ; t1 < nt ; t1++ ){
        ...
        wsp->kw[t1][t2] = (long *) xmalloc(D*ksize*sizeof(long));
    }
    ...
    for( long t1 = 0 ; t1 < nt ; t1++ ){
        for( long k = 0 ; k < ksize ; k++ ){
            ...
            long kt1t2r_ind = k + t1*strs[D] + t2*strs[D+1];
            if( wmd->data[kt1t2r_ind] > T ){
                ...
                */
                ...
                memcpy(&wsp->kw[t1][t2][D*wsp->len_w[t1][t2]], ksub, D*sizeof(long));
                wsp->len_w[t1][t2]++;
            }
        }
    }
 ...
 */
void approxBestCandidate(const int D, double *cost, MDArray<double> &deltaJ, 
